# Best seperated genes in Glucose/ Glycerol /
...
test_combined_result %>%
  ...
  dplyr::mutate(p.adj_related=glucose+glycerol,
                test_condition="carbonSource",
                overal_test_condition="carbonSource",
...
test_combined_result %>%
  dplyr::filter(test_condition %in% c("glucose","Na_mM","Mg_mM","growthTime_hr"))%>%
  ...
  dplyr::group_by(glucose,Mg_mM,Na_mM,growthTime_hr)%>%
  dplyr::summarise(numElements=length(glucose))->test_combined_resultS


test_combined_resultS %>% 
  dplyr::group_by()%>%
  dplyr::filter(glucose==1)%>%
...
test_combined_resultS %>% 
  dplyr::group_by()%>%
  dplyr::filter(glucose==1 & Mg_mM==1)%>%
  dplyr::summarize(sum(numElements))->carbon_Mg_data

test_combined_resultS %>% 
  dplyr::group_by()%>%
  dplyr::filter(glucose==1 & Na_mM==1)%>%
  dplyr::summarize(sum(numElements))->carbon_Na_data

test_combined_resultS %>% 
  dplyr::group_by()%>%
  dplyr::filter(glucose==1 & growthTime_hr==1)%>%
...
test_combined_resultS %>% 
  dplyr::group_by()%>%
  dplyr::filter(glucose==1 & Mg_mM==1 & Na_mM==1)%>%
  dplyr::summarize(sum(numElements))->carbon_Mg_Na_data

test_combined_resultS %>% 
  dplyr::group_by()%>%
  dplyr::filter(glucose==1 & Mg_mM==1 & growthTime_hr==1)%>%
  dplyr::summarize(sum(numElements))->carbon_Mg_growth_data

test_combined_resultS %>% 
  dplyr::group_by()%>%
  dplyr::filter(glucose==1 & Na_mM==1 & growthTime_hr==1)%>%
...
test_combined_resultS %>% 
  dplyr::group_by()%>%
  dplyr::filter(glucose==1 & Mg_mM==1 & Na_mM==1 & growthTime_hr==1)%>%
...
png(filename = paste0("../initialPaper03f/vennTriplet_",generalFigureName,".png")
    ,width = 1000,height = 800, units = "px")
VennDiagram::draw.triple.venn(area1 = carbonSource_data[[1]],
                              area2 = Na_mM[[1]],
                              area3 = growthTime_hr[[1]],
                              n12 = carbon_Na_data[[1]],
                              n23 = Na_growth_data[[1]],
                              n13 = carbon_growth_data[[1]],
                              n123 = carbon_Na_growth_data[[1]],
                              category = c("Carbon Source","Na Strees","Growth phase"),
                              fill = c("#decbe4","#feb24c","#ccebc5"),
                              cex = rep(3,7),
                              cat.cex = rep(3.5,3),
                              margin=0.05)
dev.off()
...
test_combined_result %>%
  ...
  dplyr::mutate(carbonSource=glucose+glycerol)->combinedHeatMap
...
fig04b<-ggplot(segment(ddata),aes(x = y, y = x, xend = yend, yend = xend)) + 
  ...
  coord_cartesian(xlim = c(5, 40))+  #Changed from 25 (like in the mRNA) to 15, indicates that the proteins are better clustered
  ylab("")+
  xlab("")+
...
test_combined_result_filtered %>% 
  ...
  dplyr::summarise(dataLength=length(test_condition))->test_combined_result_filtered_summary
rbind(test_combined_result_filtered_summary,c("Mg_mM",1,0))->test_combined_result_filtered_summary

fig05 <- ggplot(test_combined_result_filtered_summary, 
                       aes(x = test_condition, y=as.numeric(dataLength),
                           fill = as.factor(p_related_sign) )) + 
  geom_bar( position = position_dodge(),stat="identity") +
  scale_y_log10(expand = c(0, 0), limits=c(1,2000))+
  xlab("Test Condition") + ylab("Protein Count") +
