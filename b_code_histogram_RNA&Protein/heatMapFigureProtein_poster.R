...
# add dendogram.y
# add a row, one as spacer and one to take the images
g.main <- gtable_add_cols(g.main, unit.c(unit(0.3, "null")), index$l-1)
...
#add a row, one as spacer and one to take the images
index <- subset(g.main$layout, name == "panel")
g.main <- gtable_add_rows(g.main, unit.c(unit(1.25, "in")), index$b+3)
...
g.main <- gtable_add_rows(g.main, unit.c(unit(0.5, "in")), index$b-3)
...
cowplot::save_plot(plot = figComb, filename = figureName,ncol = 2.25,nrow = 3, dpi=600)
