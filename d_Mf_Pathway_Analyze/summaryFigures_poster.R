...
###*****************************
# Multilining
kegg_metabolism_summary$KEGG_Path_short=as.vector(kegg_metabolism_summary$KEGG_Path_short)
temp<-kegg_metabolism_summary$KEGG_Path_short

# temp[which(temp=="Phenylalanine, tyrosine and tryptophan biosynthesis")]<-
#   "Phenylalanine, tyrosine and tryptophan\n biosynthesis"

kegg_metabolism_summary$KEGG_Path_short=temp
###*****************************
...
fig01<-ggplot(kegg_metabolism_summary, aes( y=rank,x="condition"))+
  ...
  geom_text(aes(label=paste(rank, KEGG_Path_short, sep=". "), x=0.02),size=6, hjust=0)+
  facet_grid(vs~growthPhase+pick_data) +
  panel_border() +
...
###*****************************
# Save Figures
cowplot::save_plot(filename = paste0("../d_figures/resultTable_kegg_poster.pdf"),
                   plot = fig01,ncol = 5,nrow=3,limitsize = FALSE, dpi=600)
